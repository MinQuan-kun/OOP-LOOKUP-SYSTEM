...
const KnowledgeTree = ({ onSelectLesson }) => {
  const [treeData, setTreeData] = useState([]); // State lưu dữ liệu thật từ DB
  ...
  useEffect(() => {
    const fetchTree = async () => {
      try {
        ...
        // Mặc định mở hết các chương
        const allChapterIds = res.data.map(c => c.id);
        setExpandedNodes(allChapterIds);
  ...
  const handleLessonClick = (lessonId, slug) => {
      setActiveNode(lessonId);
      if (onSelectLesson) {
        onSelectLesson(slug); // Báo cho HomePage biết bài nào vừa được chọn
      }
  };

  if (loading) return <div className="p-4 text-gray-500">Đang tải danh mục...</div>;
                ...
                {isExpanded && (
                  <div className="ml-4 pl-3 border-l border-gray-200 mt-1 space-y-1">
                    {chapter.children.map((lesson) => (
                      <button
                        key={lesson.id}
                        // QUAN TRỌNG: Gọi hàm handleLessonClick
                        onClick={() => handleLessonClick(lesson.id, lesson.slug)}
                        className={`
